...
void loop() {
  ...
  if (currentMillis - previousMillis1 >= DECAY_FREQ) {
    ...
    growthRate = sunlightValue * 0.001 * (waterLevel / 255);
...
if (mode == 1) { // Light Level Mode
  ...
  //---------------------------------
 if (mode == 2) {
  // Map the servo value to the number of LEDs
  int numLedsLit = map(servoVal, 70, 180, 0, NUM_LEDS);
  
  // Light up the LEDs based on the servo position
  for (int i = 0; i < NUM_LEDS; i++) {
    if (i < numLedsLit) {
      leds[i] = CRGB(MAX_BRIGHTNESS, 0, 0); // LED on
    } else {
